...
const createServer = () => {
  ...
  function deleteThings(req, res){
    res.status(405)
    .catch(error = mistakeHandler(req, res, error));
  }
  ...
  function mistakeHandler(req, res, error){
    res.send(error, `we're sorry that's not right ${error}`);
  }
...
const toTitleCase = (arr) => arr.map(curr => (x)=>);
/* ------------------------------------------------------------------------------------------------
CHALLENGE 3
