...
const createServer = () => {
  ...
  app.get('/hello', (req, res) => res.send('do you aim to mis-behave? welcome to the Serenity!'));

  app.get('/aboutme', (req, res) => res.send(`I'm 32, Goth, and have a 17lb grey Tabby cat`));

  app.get('/favoritefoods', (req, res) => {
    const favFood = ['Tonight Dough', 'Pizza', 'Burritos', 'Breakfast'];
    res.send(favFood)
  });

  app.get('/',(error) => res.send(error).status(404))
...
const count = (target, input) => input.reduce((acc, curr) => (curr.filter(check => check === target).length) + acc, 0)
;
...
const totalSum = (input) => input.reduce((acc, curr) => acc + curr.reduce((acc, val) => acc + val, 0), 0);
...
FIXME: For example, [ [0,2,5,4], [2,4,10], [] ] should return [ [<1>, 32], [1024], [] ].
------------------------------------------------------------------------------------------------ */

const divisibleByFiveTwoToThePower = (input) => {

  return input.map(curr =>  
    curr.filter(num => Number.isSafeInteger(num) && num % 5 === 0)
