...
const filterStringsWithVowels = (arr) => {
  ...
  return arr.filter(curr => {
      let matchArr = curr.match(/[aeiou]/g);
...
const notInFirstArray = (forbiddenValues, arr) => {
  // Solution code here...
  const moreSlim = arr.filter((val) =>{
   const realSlim = !forbiddenValues.includes(val);
    return realSlim;
  })
  return moreSlim;
};
