...
const forLoopTwoToThe = (arr) => {
  ...
  for(let i = 0; i < arr.length; i++){
    tooPowerfull.push(Math.pow(num, arr[i]))
  }
  return tooPowerfull;
};
...
const forEachTwoToThe = (arr) => {
  ...
  arr.forEach((val, index, arr) =>{
    const num = 2;
    whyFerEach.push(Math.pow(num, val));
  })
  return whyFerEach;
};
...
const mapTwoToThe = (arr) => arr.map((current) => Math.pow(2, current));
...
const charCode = (arr) => arr.map((curr) => curr.charCodeAt());
...
const evenOdd = (arr) => arr.map((curr) => {
  if(curr % 2 === 0){
    return 'even';
  } else if(curr % 2 > 0){
    return 'odd'
