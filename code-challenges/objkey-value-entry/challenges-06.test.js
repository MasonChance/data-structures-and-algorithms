...
const templatingWithMustache = () => {
  // Solution code here...
  const arr = [];
  let output = Mustache.render("{{name}} {{spouse}} {{children}} {{house}}", $);
  // arr.push(characters.forEach(output));
  characters.forEach(current =>arr.push(output));
  return arr;
...
const hasChildrenValues = (arr, character) => {
  // Solution code here...
  const char = Object.values(arr);
  if(Object.values(arr).children && Object.values(arr).name){
    return true;
  } else {
    return false;
  }
